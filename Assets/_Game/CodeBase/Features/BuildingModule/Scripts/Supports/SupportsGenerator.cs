...
using _Game.CodeBase.Features.BuildingModule.Scripts;
...
{
    private const int FreeSpaceScanSamples = 8;
    ...
    public void PlaceSupport(WeightReceiver receiver, bool horizontal)
    {
        ...
        for (int attempt = 0; attempt < 1000; attempt++)
        {
            ...
            if (start == Vector2.zero || IsTooCloseToExistingStart(start))
            {
                ...
                {
                    // ЗМІНЕНО: без блокування — просто пробуємо апгрейд і виходимо, наступного разу спавн знову дозволений
        ...
        // ЗМІНЕНО: без блокування — просто апгрейд, стеля більше ніде не фіксується
        UpgradeRandomSupportMaterial(receiver);
    }
    ...
    private List<(float start, float end)> FindFreeSegmentsBelow(float safeMin, float safeMax, float y, Collider2D ownCollider)
    {
        var segments = new List<(float start, float end)>();

        float step = (safeMax - safeMin) / FreeSpaceScanSamples;
        if (step <= 0f) return segments;

        bool inFreeSegment = false;
        float segmentStart = safeMin;

        for (int i = 0; i <= FreeSpaceScanSamples; i++)
        {
            float x = safeMin + step * i;
            Vector2 checkPoint = new Vector2(x, y - 0.05f);
